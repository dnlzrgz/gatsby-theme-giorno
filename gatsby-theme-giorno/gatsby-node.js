...
const createPagesFromSanity = async (graphql, actions, reporter) => {
  ...
  if (res.errors) {
    reporter.panicOnBuild(`Error while running GraphQL query on createPagesFromSanity`);
    return;
  }
...
const createBlogPages = async (graphql, actions, reporter) => {
  ...
  if (res.errors) {
    reporter.panicOnBuild(`Error while running GraphQL query on createBlogPages`);
    return;
  }
...
const createPostPagesFromSanity = async (graphql, actions, reporter) => {
  ...
  const res = await graphql(`
    {
      ...
      posts: allSanityPost(filter: { public: { eq: true } }, sort: { fields: publishedAt, order: ASC }) {
        edges {
          node {
            slug {
              current
  ...
  if (res.errors) {
    reporter.panicOnBuild(`Error while running GraphQL query on createPostPagesFromSanity`);
    return;
  }
